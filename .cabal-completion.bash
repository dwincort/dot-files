...
_cabal_targets()
{
    ...
    local comp
    for comp in "$@"; do
        [ "$comp" == new-build ] && _cabal_list "name:|executable|test-suite|benchmark|library" && break
        [ "$comp" == build     ] && _cabal_list "name:|executable|test-suite|benchmark|library" && break
        [ "$comp" == new-repl  ] && _cabal_list "name:|executable|test-suite|benchmark|library" && break
        [ "$comp" == repl      ] && _cabal_list "name:|executable|test-suite|benchmark|library" && break
        [ "$comp" == new-run   ] && _cabal_list "executable"                      && break
        [ "$comp" == run       ] && _cabal_list "executable"                      && break
        [ "$comp" == new-test  ] && _cabal_list            "test-suite"           && break
        [ "$comp" == test      ] && _cabal_list            "test-suite"           && break
        [ "$comp" == new-bench ] && _cabal_list                       "benchmark" && break
